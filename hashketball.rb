# Write your code here!

require "pry"

def game_hash
    hash = {
        :home => {
            :team_name => "Brooklyn Nets",
            :colors => ["Black", "White"],
            :players => 
            [{player_name: "Alan Anderson", 
            number: 0,
            shoe: 16,
            points: 22,
            rebounds: 12,
            assists: 12,
            steals: 3,
            blocks: 1,
            slam_dunks: 1},
            {player_name: "Reggie Evans", 
            number: 30,
            shoe: 14,
            points: 12,
            rebounds: 12,
            assists: 12,
            steals: 12,
            blocks: 12,
            slam_dunks: 7
            },
            {player_name: "Brook Lopez", 
            number: 11,
            shoe: 17,
            points: 17,
            rebounds: 19,
            assists: 10,
            steals: 3,
            blocks: 1,
            slam_dunks: 15
            },
            {player_name: "Mason Plumlee", 
            number: 1,
            shoe: 19,
            points: 26,
            rebounds: 11,
            assists: 6,
            steals: 3,
            blocks: 8,
            slam_dunks: 5
            },
            {player_name: "Jason Terry", 
            number: 31,
            shoe: 15,
            points: 19,
            rebounds: 2,
            assists: 2,
            steals: 4,
            blocks: 11,
            slam_dunks: 1
            }]
        },
        away: {
            :team_name => "Charlotte Hornets",
            :colors => ["Turquoise", "Purple"],
            :players => 
            [{player_name: "Jeff Adrien", 
            number: 4,
            shoe: 18,
            points: 10,
            rebounds: 1,
            assists: 1,
            steals: 2,
            blocks: 7,
            slam_dunks: 2},
            {player_name: "Bismack Biyombo", 
            number: 0,
            shoe: 16,
            points: 12,
            rebounds: 4,
            assists: 7,
            steals: 22,
            blocks: 15,
            slam_dunks: 10
            },
            {player_name: "DeSagna Diop", 
            number: 2,
            shoe: 14,
            points: 24,
            rebounds: 12,
            assists: 12,
            steals: 4,
            blocks: 5,
            slam_dunks: 5
            },
            {player_name: "Ben Gordon", 
            number: 8,
            shoe: 15,
            points: 33,
            rebounds: 3,
            assists: 2,
            steals: 1,
            blocks: 1,
            slam_dunks: 0
            },
            {player_name: "Kemba Walker", 
            number: 33,
            shoe: 15,
            points: 6,
            rebounds: 12,
            assists: 12,
            steals: 7,
            blocks: 5,
            slam_dunks: 12
            }]
        }
    }
end

def num_points_scored(user_player)
    score = 0
    home_players = game_hash[:home][:players]
    away_players = game_hash[:away][:players]
    home_players.each do |player|
        if player[:player_name] == user_player
            score += player[:points]
        end
    end
    away_players.each do |player|
        if player[:player_name] == user_player
            score += player[:points]
        end
    end
    score
end

def shoe_size(user_player)
    size = 0
    home_players = game_hash[:home][:players]
    away_players = game_hash[:away][:players]
    home_players.each do |player|
        if player[:player_name] == user_player
            size += player[:shoe]
        end
    end
    away_players.each do |player|
        if player[:player_name] == user_player
            size += player[:shoe]
        end
    end
    size
end

def team_colors(user_team)
    home_team = game_hash[:home][:team_name]
    away_team = game_hash[:away][:team_name]
    home_colors = game_hash[:home][:colors]
    away_colors = game_hash[:away][:colors]
    colors = ""
    if home_team == user_team
        colors = home_colors
    elsif away_team == user_team 
        colors = away_colors
    end 
    colors 
end

def team_names
    array = []
    array << game_hash[:home][:team_name]
    array << game_hash[:away][:team_name]
    array
end

def player_numbers(user_team)
    array = []
    home_team = game_hash[:home][:team_name]
    away_team = game_hash[:away][:team_name]
    home_players = game_hash[:home][:players]
    away_players = game_hash[:away][:players]
    if user_team == home_team
        home_players.each do |player|
            array << player[:number]
        end 
    end
    if user_team == away_team
        away_players.each do |player|
            array << player[:number]
        end 
    end
    array
end

def player_stats(user_name)
    hash = {}
    home_players = game_hash[:home][:players]
    away_players = game_hash[:away][:players]
    home_players.each do |player|
        if player[:player_name] == user_name
            hash[:number] = player[:number]
            hash[:shoe] = player[:shoe]
            hash[:points] = player[:points]
            hash[:rebounds] = player[:rebounds]
            hash[:assists] = player[:assists]
            hash[:steals] = player[:steals]
            hash[:blocks] = player[:blocks]
            hash[:slam_dunks] = player[:slam_dunks]
        end 
    end 
    away_players.each do |player|
        if player[:player_name] == user_name
            hash[:number] = player[:number]
            hash[:shoe] = player[:shoe]
            hash[:points] = player[:points]
            hash[:rebounds] = player[:rebounds]
            hash[:assists] = player[:assists]
            hash[:steals] = player[:steals]
            hash[:blocks] = player[:blocks]
            hash[:slam_dunks] = player[:slam_dunks]
        end 
    end
    hash
end

def big_shoe_rebounds
    shoe_sizes = []
    rebounds = 0 
    biggest_shoe = 0
    home_players = game_hash[:home][:players]
    away_players = game_hash[:away][:players]
    home_players.each do |player|
        shoe_sizes << player[:shoe]
    end
    away_players.each do |player|
        shoe_sizes << player[:shoe]
    end
    sorted_sizes = shoe_sizes.sort
    biggest_shoe += sorted_sizes[-1]
    binding.pry
    home_players.each do |player|
        if player[:shoe] == biggest_shoe
            rebounds += player[:rebounds]
        end
    end
    away_players.each do |player|
        if player[:shoe] == biggest_shoe
            rebound += player[:rebounds]
        end
    end
    rebounds 
end